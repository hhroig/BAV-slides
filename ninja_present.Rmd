...
---
# *Sorry* por el *spanglish*

--

```{r fig:yoda, echo = FALSE, out.width = "70%", fig.align='center'}
knitr::include_graphics("my_images/the-spanglish-is-strong-in-this-one-it-is.jpg")
```
...
### La gramática estratificada de `ggplot`:

--

```{r ggplot-grammar, eval=FALSE}
library(ggplot2)
...
#### Pasamos al **Notebook de Visualización**
...
```{r fig:drake-ggplot , echo = FALSE, out.width = "50%", fig.align='center'}
knitr::include_graphics("my_images/drake-ggplot.jfif")
```
...
* `fctr`: categóricas o "factores" con nivels fijos (*factors*)
* `date`: fechas (*dates*) 
...
```{r fig:love-tidyverse, echo = FALSE, out.width = "60%", fig.align='center'}
knitr::include_graphics("my_images/love-tidyverse.jfif")
```
...
* Distribuciones para variables categóricas (diagramas de barras)

```{r ej:bar_chart, out.width="30%", fig.align='center'}
...
* Distribuciones para variables continuas (histogramas)

```{r ej:hist1, out.width="30%", fig.align='center'}
ggplot(data = diamonds) +
  geom_histogram(mapping = aes(x = carat), binwidth = 0.1) + # bins = ceiling(sqrt(nrow(diamonds))) 
  theme_light(base_size = 20)
```
...
* Distribuciones para variables continuas (varios histogramas superpuestos)

```{r ej:hist2, out.width="30%", fig.align='center'}
...
```{r ej:hist2_smaller, out.width="30%", fig.align='center'}
...
```{r ej:cat_vs_cont, out.width="30%", fig.align='center'}
...
* Una variable categórica y una continua
  + Es complicado ver la diferencia entre distribuciones => Usamos boxplots.

--
```{r fig:expl-boxplot, echo = FALSE, out.width = "60%", fig.align='center'}
knitr::include_graphics("my_images/eda-boxplot.png")
```
.center[
Tomado de [r4ds](https://r4ds.had.co.nz/index.html). 
]
...
* Una variable categórica y una continua (boxplots)

```{r ej:boxplot, out.width="30%", fig.align='center'}
...
```{r ej:boxplot_reorder, out.width="30%", fig.align='center'}
...
```{r ej:boxplot_flip, out.width="30%", fig.align='center'}
ggplot(data = diamonds, mapping = aes(x = reorder(cut, price, FUN = median), y = price)) +
  geom_boxplot() + 
  coord_flip() +
  theme_light(base_size = 20)
...
```{r ej:geom_count, out.width="30%", fig.align='center'}
ggplot(data = diamonds) +
...
```{r ej:geom_tile, out.width="30%", fig.align='center'}
...
```{r ej:geom_point, out.width="30%", fig.align='center'}
ggplot(data = diamonds) + 
  geom_point(mapping = aes(x = carat, y = price), alpha = 0.01, size = 2) + 
  theme_light(base_size = 20)
```
...
```{r ej:geom_point_count, out.width="30%", fig.align='center'}
...
* Si alguna método no te funciona con los tibbles:
...
* `$` es un atajo para extrar elementos con nombre de una lista. Funciona igual que `[[`, sin tener que usar las comillas: `[["var_name"]]`.
...
3. Salva tus datos "limpios" de la forma que consideres más apropriada.
...
Crear una función que calcule la varianza ($\mathbb{V}$) de un vector `x`:
...
* ### Otro my útil: `ifelse(test, yes, no)`
...
* map() hace una list.
* map_lgl() hace un vector tipo *logical*.
* map_int() hace un vector de enteros.
* map_dbl() hace un vector de *doubles*.
* map_chr() hace un vector de caracteres.
...
```{r fig:meme-fp,echo = FALSE, out.width = "50%", fig.align='center'}
knitr::include_graphics("my_images/FP_meme.png")
```
